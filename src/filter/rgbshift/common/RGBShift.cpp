...
// pointers to our data and parameters defined in RGBShift.h
Data * gData = NULL;
...
void InitData(void);
void RGBShiftRectangle(void* data,
					   int32 dataRowBytes,
					   void* mask,
					   int32 maskRowBytes,
					   VRect tileRect,
...
uint8 newPixel_8(int16 pixel, float offset);
float newPixel_f(float pixel, float offset);
uint16 newPixel_16(int32 pixel, float offset);
				...
				*gResult = gFilterRecord->advanceState();
				...
				switch (plane) {
				  case 0:
					...
					break;
				  case 1:
					color = gParams->greenOffset;
					break;
				  case 2:
					color = gParams->blueOffset;
					break;
				...
				RGBShiftRectangle(gFilterRecord->outData,
					   ...
					   int32 depth)
{
	...
	for(int32 pixelY = 0; pixelY < rectHeight; pixelY++)
	{
		for(int32 pixelX = 0; pixelX < rectWidth; pixelX++)
		{
			...
			{
				if (depth == 32){
					*fPixel = newPixel_f( *fPixel, colorOffset );
				}
				else if (depth == 16){
					*bigPixel = newPixel_16( *bigPixel, colorOffset);
				}
				else {
					*pixel = newPixel_8( *pixel, colorOffset );
				}
			}
			pixel++;
			bigPixel++;
...
uint8 newPixel_8(int16 pixel, float offset){
	pixel += (int16)( offset * 255.0 / 1000.0 );
...
uint16 newPixel_16(int32 pixel, float offset){
	pixel += (int32)( offset * 255.0 * 255.0 / 1000.0 );
	if ( pixel > 0xffff ) pixel = 0xffff;
	if ( pixel < 0 ) pixel = 0;
	return (uint16)pixel;
}
