...
extern RGBShiftController *gRGBShiftController;
...
*/
@implementation RGBShiftProxyView
...
- (id)initWithFrame:(NSRect)frame
{
    ...
    if (self)
	{
		...
		NSLog(@"RGBShift SetColor to clearColor");
    }
    return self;
}
...
- (void)viewDidChangeBackingProperties
	...
	*/
	...
	NSLog(@"RGBShift viewDidChangeBackingProperties, scale: %f", scaleFactor);
...
- (void)drawRect:(NSRect)rect
{
	...
	if ( ! [NSThread isMainThread] )
	{
		...
		[gRGBShiftController performSelectorOnMainThread:@selector(updateProxy:)
							   withObject:NULL
							waitUntilDone:NO];
		 */
	...
	NSLog(@"RGBShift drawRect, w: %f, h: %f, x: %f, y: %f, scale: %f", rect.size.width, rect.size.height, rect.origin.x, rect.origin.y, scaleFactor);
	...
	*/
	...
	[gRGBShiftController updateCursor];
}

@end

// end RGBShiftProxyView.m
