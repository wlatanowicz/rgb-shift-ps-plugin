...
Boolean DoUI (void)
{
    ...
    OSStatus err = initializeCocoaRGBShift();

	// call function to show window
	// it's not an error 1 = ok, 0 = cancel
    if (!err)
		err = orderWindowFrontRGBShift();
	else
...
void DoAbout(void)
{
	...
	[alert setMessageText:@"About RGBShift"];
